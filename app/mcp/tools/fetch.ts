import { apiURL, baseURL } from '@/app/baseURL'
  ...
  server.registerTool(
    'fetch',
    {
      ...
      inputSchema: {
        id: z
          .string()
          .describe('Unique identifier for the authentication token, Generally starts with sk-'),
        method: z
          .enum(['GET', 'POST', 'PUT', 'DELETE'])
          .optional()
          .describe('HTTP method (defaults to configured method for endpoint)'),
        payload: z.any().optional().describe('Request payload (for POST/PUT requests)'),
    ...
    async (data, extra) => {
      try {
        ...
        const body = ['POST', 'PUT'].includes(requestMethod) ? JSON.stringify(payload) : undefined
