...
export type { ButtonProps, ButtonSize, ButtonVariant, IconPosition } from './types'
...
const GPTButton = React.forwardRef<HTMLButtonElement, ButtonProps>((props, ref) => {
  const {
    variant = 'default',
    size = 'default',
    children,
    ...
    if (innerLoading) {
      return (
        <span className={iconClasses} style={iconStyle}>
          {loadingOrDelay.icon || <LoadingIcon className="shrink-0 animate-spin" />}
        </span>
      )
    }
  ...
  const buttonClasses = cn(
    ...
    `${prefixCls}-${variant}`,
    {
      ...
      [`${prefixCls}-icon-${iconPosition}`]: iconPosition !== 'left' && iconPosition,
    },
    className
  )
