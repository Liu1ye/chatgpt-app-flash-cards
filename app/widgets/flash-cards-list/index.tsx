...
import { ArrowRightLgIcon, ArrowLeftLgIcon, FolderOpenIcon } from '@/app/assets/icons'
import GPTButton from '@/app/components/GPTButton'
import { FlashCardData, FlashCardDataItem, FlashCardsResponse } from '../types'
...
const FlashCardsList = () => {
  ...
  const [dataList, setDataList] = useState<FlashCardData[]>([])
  const [fetchLoading, setFetchLoading] = useState(false)
  const callTool = useCallTool()
  const { t } = useTranslation()

  // 计算总页数
  const totalPages = useMemo(() => Math.ceil(total / FRONTEND_PAGE_SIZE), [total])
    ...
    async (backendPage: number) => {
      try {
        ...
        if (res?.structuredContent?.response?.code === 0) {
          const data = res.structuredContent.response.data as FlashCardsResponse
  ...
  }, [currentPage, totalPages, dataList.length, getRequiredBackendPage, getDataList])
  ...
  return (
    <div className="flex flex-col w-full h-[475px] bg-bg-primary">
      <div className="p-[16px_10px_16px_16px]">
        <span className="text-text-primary text-[17px] font-[500]">{t('card.my-card')}</span>
        <span className="text-text-secondary text-[14px] font-[500]"> ({total})</span>
      </div>
      <div className="flex-1">
        {dataList.length > 0 ? (
          dataList.map((item) => <ListItem key={item.id} item={item} onClick={handleClickItem} />)
        ) : (
          <div className="flex-center flex-col gap-y-4 size-full">
            ...
            <span className="text-[14px] text-text-secondary">{t('card.no-file')}</span>
          </div>
        )}
      </div>
      {dataList.length > 0 ? (
